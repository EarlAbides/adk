<?php
	...
	class HikersPeaks {
		...
		public function getFirstPeaks($con) {
			$sql_query = sql_getHikersFirstPeaks($con, $this->userid);
			if($sql_query->execute()){
				$sql_query->store_result();
				$result = sql_get_assoc($sql_query);
				...
				foreach($result as $row){
					$ADK_HIKERS_PEAK = new HikersPeak();
					$ADK_HIKERS_PEAK->id = $row['ADK_PEAK_ID'];
					$ADK_HIKERS_PEAK->peakname = $row['ADK_PEAK_NAME'];
					$ADK_HIKERS_PEAK->datetime = $row['ADK_PEAK_DTE'];
					array_push($this->hikerspeaks, $ADK_HIKERS_PEAK);
				}
			}
			else die('There was an error running the query ['.$con->error.']');		
