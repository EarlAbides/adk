<?php
	...
	function sql_getHikers($con, $ADK_HIKER_CORR_ID){
		$sql_query = $con->prepare(
            "SELECT A.ADK_USER_ID, B.ADK_USER_USERNAME, B.ADK_USER_NAME, B.ADK_USER_EMAIL, A.ADK_HIKER_CORR_ID, A.ADK_HIKER_LASTACTIVE_DTE
		...
		$sql_query = $con->prepare(
            "SELECT H.ADK_USER_ID, H.ADK_HIKER_CORR_ID, H.ADK_HIKER_PHOTO_ID, U.ADK_USER_USERNAME, U.ADK_USER_NAME, U.ADK_USER_EMAIL, H.ADK_HIKER_PHONE
				...
				,H.ADK_HIKER_ZIP, H.ADK_HIKER_COUNTRY, H.ADK_HIKER_PERSONALINFO, H.ADK_HIKER_LASTACTIVE_DTE
					,(SELECT COUNT(*) FROM ADK_HIKE_PEAK_JCT C
						LEFT JOIN ADK_HIKE D ON C.ADK_HIKE_ID = D.ADK_HIKE_ID
					WHERE H.ADK_USER_ID = D.ADK_USER_ID) AS ADK_HIKER_NUMPEAKS
			FROM ADK_HIKER H
				LEFT JOIN ADK_USER U ON H.ADK_USER_ID = U.ADK_USER_ID
			WHERE U.ADK_USER_ID = ?;"
        );
