<?php
	...
	function sql_getApplicants($con){
		$sql_query = $con->prepare(
            "SELECT ADK_APPLICANT_ID, ADK_APPLICANT_USERNAME, ADK_APPLICANT_NAME, ADK_APPLICANT_EMAIL,
					...
					ADK_APPLICANT_COUNTRY, ADK_APPLICANT_PERSONALINFO, ADK_APPLICANT_FIRSTPEAK_ID,
					ADK_APPLICANT_FIRSTPEAK_DTE, ADK_APPLICANT_NUMPEAKS, ADK_APPLICANT_REQ_CORR
				FROM ADK_APPLICANT;"
        );
	...
	function sql_getApplicant($con, $ADK_CORRESPONDENT_ID){
		$sql_query = $con->prepare(
            "SELECT A.ADK_APPLICANT_ID, A.ADK_APPLICANT_USERNAME, A.ADK_APPLICANT_NAME, A.ADK_APPLICANT_EMAIL, A.ADK_APPLICANT_PHONE,
				...
				A.ADK_APPLICANT_PERSONALINFO, A.ADK_APPLICANT_FIRSTPEAK_ID, B.ADK_PEAK_NAME AS ADK_APPLICANT_FIRSTPEAK_NAME,
				A.ADK_APPLICANT_FIRSTPEAK_DTE, ADK_APPLICANT_NUMPEAKS, ADK_APPLICANT_REQ_CORR
			FROM ADK_APPLICANT A
				LEFT JOIN ADK_PEAK B ON A.ADK_APPLICANT_FIRSTPEAK_ID = B.ADK_PEAK_ID
	...
	function sql_getHiker($con, $ADK_USER_ID){
		$sql_query = $con->prepare(
            "SELECT A.ADK_USER_ID, B.ADK_USER_USERNAME, B.ADK_USER_NAME, B.ADK_USER_EMAIL, A.ADK_HIKER_CORR_ID, A.ADK_HIKER_PHONE,
				...
				A.ADK_HIKER_ZIP, A.ADK_HIKER_COUNTRY, A.ADK_HIKER_PERSONALINFO, A.ADK_HIKER_FIRSTPEAK_ID, C.ADK_PEAK_NAME AS ADK_HIKER_FIRSTPEAK_NAME, 
                A.ADK_HIKER_FIRSTPEAK_DTE, A.ADK_HIKER_LASTPEAK_ID, A.ADK_HIKER_LASTPEAK_DTE,
					(SELECT COUNT(*) FROM ADK_HIKE_PEAK_JCT C
						LEFT JOIN ADK_HIKE D ON C.ADK_HIKE_ID = D.ADK_HIKE_ID
					WHERE A.ADK_USER_ID = D.ADK_USER_ID) AS ADK_HIKER_NUMPEAKS
			FROM ADK_HIKER A
	...
	function sql_getNewMessageCount($con, $ADK_USER_ID){
		$sql_query = $con->prepare("SELECT COUNT(*) AS COUNT FROM ADK_MESSAGE WHERE ADK_MESSAGE_TO_USER_ID = ? AND ADK_MESSAGE_READ = '0';");
