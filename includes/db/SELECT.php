<?php
	...
	function sql_getHiker($con, $ADK_USER_ID) {
		$sql_query = $con->prepare(
            "SELECT H.ADK_USER_ID, H.ADK_HIKER_CORR_ID, H.ADK_HIKER_PHOTO_ID, U.ADK_USER_USERNAME, U.ADK_USER_NAME, U.ADK_USER_EMAIL, H.ADK_HIKER_PHONE
				...
				, H.ADK_HIKER_ZIP, H.ADK_HIKER_COUNTRY, H.ADK_HIKER_PERSONALINFO, H.ADK_HIKER_LASTACTIVE_DTE, H.ADK_HIKER_DTE, H.ADK_HIKER_COMPLETE_DTE
	...
	function sql_batch_getNewHikersWhoFinished46($con) {
		$sql_query = $con->prepare(
			...
			FROM ADK_USER U
				LEFT JOIN ADK_HIKER H ON H.ADK_USER_ID = U.ADK_USER_ID
			WHERE H.ADK_HIKER_COMPLETE_DTE > DATE_ADD(CURRENT_TIMESTAMP, INTERVAL -3 MONTH);"
		);
	...
	function sql_batch_getHikersCorrespondenceHistory($con, $ADK_USER_ID) {
		$sql_query = $con->prepare(
            "SELECT ADK_MESSAGE_ID, ADK_MESSAGE_FROM_USER_ID, ADK_MESSAGE_TO_USER_ID, ADK_MESSAGE_RESPOND_ID, ADK_MESSAGE_TITLE, ADK_MESSAGE_CONTENT, ADK_MESSAGE_DTE
				, (SELECT ADK_USER_USERNAME FROM ADK_USER U WHERE U.ADK_USER_ID = M.ADK_MESSAGE_FROM_USER_ID) ADK_MESSAGE_FROM_USERNAME
				, (SELECT ADK_USER_NAME FROM ADK_USER U WHERE U.ADK_USER_ID = M.ADK_MESSAGE_FROM_USER_ID) ADK_MESSAGE_FROM_NAME
				, (SELECT ADK_USER_USERNAME FROM ADK_USER U WHERE U.ADK_USER_ID = M.ADK_MESSAGE_TO_USER_ID) ADK_MESSAGE_TO_USERNAME
				, (SELECT ADK_USER_NAME FROM ADK_USER U WHERE U.ADK_USER_ID = M.ADK_MESSAGE_TO_USER_ID) ADK_MESSAGE_TO_NAME
			FROM ADK_MESSAGE M
			WHERE (ADK_MESSAGE_FROM_USER_ID = ? OR ADK_MESSAGE_TO_USER_ID = ?)
				AND ADK_MESSAGE_FROM_DELETED = 0
				AND ADK_MESSAGE_DRAFT = 0
			ORDER BY ADK_MESSAGE_DTE;"
        );
