<?php
	...
	function sql_getApplicants($con){
		$sql_query = $con->prepare(
			"SELECT ADK_APPLICANT_ID, ADK_APPLICANT_USERNAME, ADK_APPLICANT_NAME, ADK_APPLICANT_EMAIL, ADK_APPLICANT_PHONE, ADK_APPLICANT_STATE
				,SUBTIME(ADK_APPLICANT_DTE,'0 5:00:00.00') ADK_APPLICANT_DTE
			FROM ADK_APPLICANT;"
		);
	...
	function sql_getApplicant($con, $ADK_APPLICANT_ID){
		...
		$sql_query = $con->prepare(
			"SELECT A.ADK_APPLICANT_ID, A.ADK_APPLICANT_USERNAME, A.ADK_APPLICANT_NAME, A.ADK_APPLICANT_EMAIL, A.ADK_APPLICANT_PHONE
				...
				,A.ADK_APPLICANT_PERSONALINFO, A.ADK_APPLICANT_REQ_CORR, A.ADK_APPLICANT_PEAKIDS
				,(SELECT GROUP_CONCAT(' ', ADK_PEAK_NAME) FROM ADK_PEAK WHERE ADK_PEAK_ID IN(
					SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(t.ADK_APPLICANT_PEAKIDS, ',', n.n), ',', -1) v
                    FROM ADK_APPLICANT t CROSS JOIN(
						SELECT a.N + b.N * 10 + 1 n FROM (SELECT 0 AS N UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9) a
							,(SELECT 0 AS N UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9) b ORDER BY n) n
                    WHERE n.n <= 1 + (LENGTH(t.ADK_APPLICANT_PEAKIDS) - LENGTH(REPLACE(t.ADK_APPLICANT_PEAKIDS, ',', '')))
						AND t.ADK_APPLICANT_ID = A.ADK_APPLICANT_ID
                    ORDER BY v
	...
	function sql_getApplicantPeakList($con, $ADK_APPLICANT_PEAKIDS){
		$ADK_APPLICANT_PEAKIDS = implode(',',  $ADK_APPLICANT_PEAKIDS);
		$sql_query = $con->prepare("SELECT GROUP_CONCAT(' ', ADK_PEAK_NAME) FROM ADK_PEAK WHERE ADK_PEAK_ID IN($ADK_APPLICANT_PEAKIDS);");
	...
	function sql_getCorrespondents($con){
		$sql_query = $con->prepare(
            "SELECT A.ADK_USER_ID, B.ADK_USER_USERNAME, B.ADK_USER_NAME, B.ADK_USER_EMAIL, A.ADK_CORR_PHOTO_ID,
				...
				,SUBTIME(ADK_CORR_DTE,'0 5:00:00.00') ADK_CORR_DTE
	...
	function sql_getHikes($con, $ADK_USER_ID){
		$sql_query = $con->prepare(
            "SELECT A.ADK_HIKE_ID, A.ADK_HIKE_NOTES, A.ADK_HIKE_DTE
				...
				, SUBTIME(ADK_HIKE_TS,'0 5:00:00.00') ADK_HIKE_TS
			FROM ADK_HIKE A
			WHERE A.ADK_USER_ID = ?;"
        );
	...
	function sql_getHikers($con, $ADK_HIKER_CORR_ID){
		$sql_query = $con->prepare(
            "SELECT H.ADK_USER_ID, U.ADK_USER_USERNAME, U.ADK_USER_NAME, U.ADK_USER_EMAIL, H.ADK_HIKER_CORR_ID, H.ADK_HIKER_LASTACTIVE_DTE
				, SUBTIME(H.ADK_HIKER_DTE,'0 5:00:00.00') ADK_HIKER_DTE
	...
	function sql_getHiker($con, $ADK_USER_ID){
		$sql_query = $con->prepare(
            "SELECT H.ADK_USER_ID, H.ADK_HIKER_CORR_ID, H.ADK_HIKER_PHOTO_ID, U.ADK_USER_USERNAME, U.ADK_USER_NAME, U.ADK_USER_EMAIL, H.ADK_HIKER_PHONE
				...
				, H.ADK_HIKER_ZIP, H.ADK_HIKER_COUNTRY, H.ADK_HIKER_PERSONALINFO, H.ADK_HIKER_LASTACTIVE_DTE
				, SUBTIME(H.ADK_HIKER_DTE,'0 5:00:00.00') ADK_HIKER_DTE
